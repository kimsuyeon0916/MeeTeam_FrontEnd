	...
	return (
		<S.MemberContactLayout>
			<div className='member-contact__tag-row'>
				{member.authority === '리더' ? (
					<S.MemberContactTag>{member.authority}</S.MemberContactTag>
				) : (
					<S.MemberContactTag $color='#E4F9D0'>{member.authority}</S.MemberContactTag>
				)}
				<S.MemberContactTag $color={member.role?.[1]}>{member.role?.[0]}</S.MemberContactTag>
			</div>
							...
							{member.email === '' ? (
								...
								<>
									...
									<S.MemberContactCopyLink
										alt='클립보드 복사'
										src={copyLinkIcon}
										onClick={() => CopyClipBoard(member.email)}
									/>
								</>
							)}
						</div>
